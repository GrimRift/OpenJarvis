 ...
 * keeps inflating reads as restless. Speaking does not breathe at all -- its
 * size is the voice, which is a different motion and fights a cycle running
 * underneath it. */
...
export function speakingScale(voiceEnv: number): number {
  const rest = PLEXUS_STATES.away.r;
