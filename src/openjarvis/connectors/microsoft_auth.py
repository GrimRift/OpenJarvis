"""Shared Microsoft OAuth helpers: access token read + one-shot 401 refresh.

Mirrors ``google_auth.py``. Currently only the Outlook Mail connector uses
this, but the shape (and the ``microsoft.json`` shared credentials file)
exists so a future Calendar/OneDrive/Teams connector can reuse the same
refresh wrapper instead of duplicating it.
...
class MicrosoftAuthError(RuntimeError):
    """Raised when Microsoft credentials are missing or refresh-token grant fails."""
...
def refresh_access_token(credentials_path: str) -> str:
    ...
    Returns the new access_token. Raises :class:`MicrosoftAuthError` when the
    credentials file is missing, lacks a refresh_token / client credentials,
    or when Microsoft rejects the refresh grant (e.g. the refresh_token has
    ...
    # Microsoft rotates refresh tokens on use; persist the new one if given.
    if payload.get("refresh_token"):
        tokens["refresh_token"] = payload["refresh_token"]
    save_tokens(credentials_path, tokens)
    logger.info(
        "Refreshed Microsoft access token (expires_in=%s)", payload.get("expires_in")
    )
    return new_token
...
def call_with_refresh(
    ...
    **kwargs: Any,
    ...
    Microsoft Graph returns 401 (the access token has expired or been
    revoked) uses the stored refresh_token to mint a new access_token,
    updates the credentials file, and retries the call exactly once.

    Any other ``HTTPStatusError`` is re-raised unchanged — auth-related
    retries end here; transient 5xx / timeout retries belong further up
    the stack.
    """
    ...
    try:
        ...
        if exc.response is None or exc.response.status_code != 401:
            raise
        logger.info(
            "Microsoft Graph returned 401 on %s — refreshing access token"
            " and retrying.",
