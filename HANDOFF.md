...
- **M32 is NOT closed: the action layer was deliberately removed (2026-09-02).**
  ...
  - **So M32's remaining scope is a design question, not a re-add.** Reinstating `click_control` as it was would reintroduce exactly what was removed. The open question is which apps genuinely have no API worth driving, and whether UI Automation's `Invoke()` on a named element is worth it for those alone. The user has confirmed M32 stays open.
