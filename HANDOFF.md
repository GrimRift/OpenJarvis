...
- **Voice turns no longer put a 3.6 GB draft model on the GPU (2026-09-02).**
  ...
  - **Still latent, not introduced here and not fixed here:** *any* socket drop between `beginTurn` and `EndOfTurn` strands `fluxTurnActive` and kills wake-word re-arm until reload. The reconnect path has no way to tell `InputArea` to clear it. Worth doing, deliberately left out of this change.
