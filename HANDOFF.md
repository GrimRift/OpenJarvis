...
output-latency tail, long tool-assisted conversations remain provider-valid
after trimming, and the Chat/Voice-local wake-word listener self-recovers from
stale rearm sessions and idle capture stalls. The only pending confirmation in
this voice slice is one real-microphone wake/silence/rearm cycle after the latest
liveness fix.
